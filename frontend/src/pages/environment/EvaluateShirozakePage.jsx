...
const itemsBreadcrumb = [
    ...
    { title: <Link to={'/vesinh/evaluate-shirozake'}>Shirozake</Link> },
];

const EvaluateShirozakePage = () => {
    ...
    const insertEvaluate = async () => {
        if (Object.values(evaluateValue).length === evaluate.length) {
            try {
                ...
                navigate('/vesinh/evaluate-bottling');
            } catch (error) {
                setModalError({ error, open: true });
            } finally {
                setLoading(false);
...
export default EvaluateShirozakePage;
